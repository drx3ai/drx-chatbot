...
class IntelligentRequestOrchestrator {
  ...
  private async callOpenAI(prompt: string): Promise<any> {
    // Use OpenAI's chat completions API with the GPT‑5 model.  This endpoint
    // accepts an array of messages similar to the chat API used elsewhere in
    // this project.  We send a single user message.
    ...
    return {
      // Extract the assistant's reply from the first choice.  The chat API
      // returns an array of choices, each containing a message with a `content`.
