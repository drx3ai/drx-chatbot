...
export async function POST() {
  try {
    ...
    const results = await Promise.allSettled([
      aiOrchestrator.generateResponse(testMessage, { provider: "openai" }),
      aiOrchestrator.generateResponse(testMessage, { provider: "deepseek" }),
      aiOrchestrator.generateResponse(testMessage, { provider: "deepseek" }),
    ])

    const testData = results.map((result, index) => {
      const providers = ["openai", "deepseek", "deepseek"]
      return {
        provider: providers[index],
        status: result.status,
        success: result.status === "fulfilled" ? (result as any).value.success : false,
        response: result.status === "fulfilled" ? (result as any).value.response?.substring(0, 100) : null,
        error:
          result.status === "rejected"
            ? (result as any).reason.message
